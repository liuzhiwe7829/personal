...
public class ConfigCenterDemo {
    ...
    /**
     * 需要配置的服务器都从zk上取，并注册watch来实现实时获得配置更新
     */
    public void getConfigFromZk(){
        ZkClient client = new ZkClient("localhost");
        client.setZkSerializer(new MyZkSerializer());
        String configPath = "/config1";
        String value = client.readData(configPath);
        System.out.println("从zk读取到配置config的值："+value);
        //监控配置的更新
        ...
        //这里只是为演示实时获取到配置值更新而加的等待，实际项目应用中根据具体场景写（可用塞方式）
        try{
